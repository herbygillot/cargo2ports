...
Simple Python utility to generate a `cargo.crates` stanza for a MacPorts Portfile from a Rust project's
V2 lockfile.

Taking in a `Cargo.lock` file that looks like this:
...
`cargoV2forports.py -i 2 ./Cargo.lock`